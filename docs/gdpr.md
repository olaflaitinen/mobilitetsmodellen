...
Only the variables necessary for intergenerational mobility estimation are retained in
analysis files. Raw register data is accessed in the SCB MONA/SAFE environment and is
never exported or committed.

## Security of Processing (Article 32)

- All register data remains within the MONA/SAFE environment.
- No personal data, microdata, or kinship graphs are committed to this repository.
- Gitleaks rules in `.gitleaks.toml` enforce personnummer and organisationsnummer detection.
...
- Minimum cell-size suppression is applied to municipality-level outputs.
